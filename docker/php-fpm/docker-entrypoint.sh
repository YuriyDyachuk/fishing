...
if [ ! -d "/var/www/html/backups" ]; then
	run "mkdir -p /var/www/html/backups" "${DEBUG_LEVEL}"
fi
if [ ! -d "/var/www/html" ]; then
	run "mkdir -p /var/www/html" "${DEBUG_LEVEL}"
fi
run "chown ${MY_USER}:${MY_GROUP} /var/www/html/backups" "${DEBUG_LEVEL}"
run "chown ${MY_USER}:${MY_GROUP} /var/www/html" "${DEBUG_LEVEL}"
run "chmod 0755 /var/www/html/backups" "${DEBUG_LEVEL}"
run "chmod 0755 /var/www/html" "${DEBUG_LEVEL}"
